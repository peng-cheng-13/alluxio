...
JobTransforms::transformJob(ClassAd *ad, CondorError * /* errorStack */ )
{
	...
	//Author: Peng Cheng: Data-aware task scheduling module
        dprintf(D_ALWAYS,"JOB_TRANSFORM is enabled\n");
	gettimeofday(&tv,NULL);
        //Disable Data-aware by defalut
        ...
        char* filehost = NULL;
        int i = -1;
        ad->LookupString(ATTR_JOB_INPUT, &inputbuf);
	if(inputbuf != NULL)
		dprintf(D_ALWAYS,"ATTR_JOB_INPUT = %s \n",inputbuf);

        ad->LookupBool(ATTR_DATA_AWARE, data_aware);

        if(!data_aware)
                dprintf(D_ALWAYS,"Data aware moudle is disabled\n");

        /*StringList input_paths(inputbuf,",");
	path_buf=input_paths.next();
	if(path_buf != NULL)
		 dprintf(D_ALWAYS,"Input file path = %s \n",path_buf);
	else
		 dprintf(D_ALWAYS,"Input file path is NULL\n");
	*/
	
        //Get the host of data
        if ((inputbuf != NULL) && data_aware ){
		dprintf(D_ALWAYS,"Enable Data aware moudle\n");
		string spath=inputbuf;
		if(spath.find("/home/condor/alluxio-data")<spath.length()){
			if(access(inputbuf,0)==0)
				filehost = gethost_c(inputbuf);
			else
				dprintf(D_ALWAYS,"File does not exist in TDMS \n");
		}

                if(filehost != NULL)
                	dprintf(D_ALWAYS,"Input file host = %s \n",filehost);
		else
			dprintf(D_ALWAYS,"gethost for input file but return NULL \n");
        }

        //Set Job ClassAd
        if((filehost != NULL) && data_aware )
                i = SetAttributeString(cluster,proc, ATTR_JOB_MACHINE_ATTRS, filehost);

        if(i==-1)
                dprintf(D_ALWAYS,"Set ATTR_JOB_MACHINE_ATTRS Failed\n");
	else
		dprintf(D_ALWAYS,"Set ATTR_JOB_MACHINE_ATTRS %s Succeed\n",filehost);	

	gettimeofday(&te,NULL);
	dprintf(D_ALWAYS,"Time spent in DA module = %ld us\n",1000000*(te.tv_sec-tv.tv_sec)+ te.tv_usec-tv.tv_usec);
        //End of Data-aware task scheduling module
